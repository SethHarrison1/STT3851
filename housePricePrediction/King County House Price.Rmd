...
```{r}
mod5 <- earth(price ~ . - sqft_lot15 - sqft_basement + condition:grade, data = housedata, degree = 2)
mod5
```
...
```{r}
PredictedPrice5 <- predict(mod5, newdata = housedataT)
head(PredictedPrice5)
formatSubmission <- data.frame(id = 1:4229, price = PredictedPrice5)
head(formatSubmission)
write.csv(formatSubmission, "Example5.csv", row.names  = FALSE)
```
