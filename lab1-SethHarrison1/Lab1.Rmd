...
To display the saved file as shown in Figure \@ref(fig:fig1), use the `include_graphics()` function from `knitr`.^[Use argument `dpi = 96` inside `include_graphics()`.]

```{r, label = "fig1", fig.cap = "Using `knitr::include_graphics()`", echo = FALSE}
library(knitr)
...
We will not create some more sophisticated plots. The `countour()` function produces a contour plot in order to represent the three-dimensional data; it is like a topographical map. It takes three arguments:

1. A vector of the x values(the first dimension),
2. A vector of the y values(the second dimension), and
3. A matrix whos elements correspond to the z value(the third dimension) for each pair of (x,y) coordinates.
  
As with the `plot()` function, there are many other inputs that can be used to fine-tune the output of the `countour()` function. To learn more about these, take a look at the help file by typing `?contour`.
...
```{r}
site <- "http://faculty.marshall.usc.edu/gareth-james/ISL/Auto.data"
...
```{r}
site <- "http://faculty.marshall.usc.edu/gareth-james/ISL/Auto.csv"
...
See the `geom_boxplot` [documentation](https://ggplot2.tidyverse.org/reference/geom_boxplot.html) and the `geom_freqpoly` [documentation](https://ggplot2.tidyverse.org/reference/geom_histogram.html) for more details.
```{r}
library(ggplot2)
