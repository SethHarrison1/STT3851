...
set.seed(389)
library(knitr)
knitr::opts_chunk$set(fig.show = 'as.is', fig.height = 4, fig.width = 4, prompt = FALSE, highlight = TRUE, tidy = FALSE, warning = FALSE, message = FALSE, fig.align = "center", comment = NA)
```
...
cv <- numeric(k)
  ...
  lines(1:10, cv, col = reps)
  points(1:10, cv, col = reps, pch = 19)
  }
## GGplot2 approach
set.seed(123)
cv <- matrix(NA, 10, 9)
...
MSPE <- numeric(k)
 for(reps in 1:9){
    ...
    folds <- sample(x = 1:k, size = nrow(Auto), replace = TRUE)
      ...
      cv[i, reps] <- weighted.mean(MSPE, table(folds)/sum(folds))
    }
 }
NDF2 <- data.frame(MSPE = as.vector(cv), degree = rep(1:10, 9), fold = rep(1:9, each = 10))
ggplot(NDF2, aes(x = degree, y = MSPE, group = fold, color = as.factor(fold))) +
  geom_line() +
  geom_point() +
  theme_bw() +
  ylim(15,30) +
  scale_x_continuous(breaks = 1:10) +
  labs(x = "Degree of the Polynomial", y = "Mean Squared Prediction Error") +
  guides(color = FALSE)
```
...
for(j in 1:9){
  ...
  lines(1:10, cv.err, col = j)
  points(1:10, cv.err, col = j, pch = 19)
}
# GGplot2 approach
#
#
...
k <- nrow(HSWRESTLER)
MSPE <- numeric(k)
folds <- sample(x = 1:k, size = nrow(HSWRESTLER), replace = F)
for(j in 1:k){
  modf <- lm(hwfat ~ abs + triceps, HSWRESTLER[folds != j,])
